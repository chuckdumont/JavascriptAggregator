...
public class CacheImpl implements ICache {
	...
	private ConcurrentMap<String, IGenericCache> _genericCaches;
	...
	public CacheImpl(IAggregator aggregator, Object control, ICache oldCache) {
		...
		_genericCaches = new ConcurrentHashMap<String, IGenericCache>();
		// If there are any generic cache objects in the old cache , then create new instances
		// of those in the new cache.
		if (oldCache != null) {
			ConcurrentMap<String, IGenericCache> old = ((CacheImpl)oldCache)._genericCaches;
			for (Map.Entry<String, IGenericCache> entry : old.entrySet()) {
				_genericCaches.put(entry.getKey(), entry.getValue().newInstance());
			}
		}
		_control = control;
	...
	public IGenericCache putIfAbsent(String key, IGenericCache cache) {
		return _genericCaches.putIfAbsent(key, cache);
	}
	...
	public boolean remove(String key) {
		return _genericCaches.remove(key) != null;
	}
	@Override
	...
	public synchronized void clear() {
		...
		_moduleCache.clear();
		_gzipCache.clear();
		for (IGenericCache cache : _genericCaches.values()) {
			cache.clear();
		}
	}
